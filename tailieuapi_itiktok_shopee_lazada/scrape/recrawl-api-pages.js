/*
 * Re-crawl API Reference pages that have thin content (< 500 chars)
 * because the scribe-feedback container was mistakenly removed
 */
const fs   = require('fs');
  ...
  return await page.evaluate(
    ({ contentSels, removeSels, fallback }) => {
      ...
      for (const s of contentSels) {
        ...
        if (el && (el.textContent || '').trim().length > 100) { node = el; break; }
      }
      if (!node) node = document.body;

      const clone = node.cloneNode(true);
      removeSels.forEach(s => clone.querySelectorAll(s).forEach(e => e.remove()));
      // Remove base64 images inline
...
(async () => {
  // Load manifest to find thin API pages
  ...
  console.log(`Found ${thinPages.length} thin API Reference pages to re-crawl`);
  ...
  for (let i = 0; i < thinPages.length; i++) {
    ...
    try {
      ...
      if (mEntry) {
        mEntry.textLen = content.textLen;
        mEntry.title = title;
        mEntry.file = file;
      }

      fixed++;
      console.log(`[${i+1}/${thinPages.length}] FIXED ${content.textLen}ch ${p.url.split('/').pop()}`);
