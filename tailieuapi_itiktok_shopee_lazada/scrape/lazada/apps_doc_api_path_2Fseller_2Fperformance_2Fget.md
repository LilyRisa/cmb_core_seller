...
> Scraped: 2026-05-20T22:45:36.682Z
...
Indicator type (e.g. POSITIVE\_SELLER\_RATING, PRODUCT\_RATING\_COVERAGE, ...).
...
Name of the indicator is the seller's language.
...
Longer description of the indicator is the seller's language.
...
Raw score value. Note: if the indicator doesn't contain any value, a null value is set instead.
...
Score format: INTEGER, DOUBLE, PERCENTAGE, MINUTES, HOURS.
...
Score formatted in the seller's language and locale. Note: if the indicator doesn't contain any value, a "-" is set instead.
...
Indicator target (raw value). Note: if the indicator doesn't contain any value, a null value is set instead.
...
Target format: GREATER\_THAN\_DOUBLE ('≥' #.##), GREATER\_THAN\_PERCENTAGE ('≥' #.##'%'), LOWER\_THAN\_PERCENTAGE('≤' #.##'%'), LOWER\_THAN\_MINUTES('≤' #'min'), STRICTLY\_LOWER\_THAN\_HOURS('<' #'h'), GREATER\_THAN\_DOUBLE ('≥' #.##), EQUALS\_TO\_INTEGER(= #).
...
Indicator target formatted in the seller's language and locale.
...
true if the formattedScore respects the formattedTarget, false if not.
...
Relative (from the Seller Portal) or absolute URL to redirect the seller to the page where he cans handle the task.
