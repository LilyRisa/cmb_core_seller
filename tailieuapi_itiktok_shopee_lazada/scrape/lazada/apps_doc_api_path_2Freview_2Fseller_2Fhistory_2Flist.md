...
> Scraped: 2026-05-20T22:57:22.650Z
...
Start Time, timestamp in millisecond, this is the same with "create\_time" in the response data of interface (/review/seller/list/v2)；The time range cannot exceed 7 days
...
End Time, timestamp in millisecond, this is the same with "create\_time" in the response data of interface (/review/seller/list/v2)；The time range cannot exceed 7 days
...
CURRENT\_ABOVE\_LIMIT

 | 

"current" is above the limit, the max value is 50
...
NULL\_STARTTIME\_OR\_ENDTIME

 | 

Cannot recognize "start\_time" or "end\_time"
...
STARTTIME\_OVER\_LIMIT

 | 

Only support checking 90 days of history data
...
TIMESPAN\_ABOVE\_LIMIT

 | 

Only support checking 7days data at one time
...
start\_time&end\_time range cannot exceed 7 days.
...
Mp3 Seller not support the api -apipath

 | 

MP3 sellers cannot call the current API, please readthis document for a list of APIs that can be called by MP3 sellers, and you can call the GetSeller API and check the marketplaceEaseMode field to confirm that the current seller is of type MP3.
