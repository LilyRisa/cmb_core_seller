...
> Scraped: 2026-05-20T22:48:36.881Z
...
[Parameter description](https://open.lazada.com/apps/doc/doc?nodeId=30720&docId=120949)
...
The status of product created, including Active, InActive, and Pending QC. if a product is in Pending status, it needs to be reviewed and will be processed within 24 hours.
...
E205: SPU does not exist

 | 

The specified SPU ID does not exist.
...
E206: Different category id in SPU and PrimaryCategory

 | 

The specified category ID is not consistent.
...
E502: Search SPU failed

 | 

Failed to search for the specified SPU.
...
Please select the last level category.
...
The image exists in the outer link
...
The brand is invalid in the category
...
THD\_IC\_ERR\_F\_IC\_SERVICE\_EDIT\_002
...
BIZ\_CHECK\_ITEM\_HAS\_REACH\_LIMIT

 | 

Seller's online product count has reach limit
...
BIZ\_CHECK\_PACKAGE\_DECIMAL\_INVALID

 | 

Package attribute value is not valid
...
BIZ\_CHECK\_MTEE\_RULE\_QC

 | 

Quality Check, Failed by Lazada Policy
...
THD\_INVENTORY\_ERR\_INV\_PARAM\_ILLEGAL
...
THD\_IC\_ERR\_FC\_IC\_SKU\_IMAGE\_001
...
SYS\_REQUEST\_TOO\_FAST

 | 

Slow down a bit! Too many opertions at once. Please try again later
...
BIZ\_CHECK\_NO\_EDIT\_ITEM\_LOCK

 | 

This product is currently locked so you are unable to do editing. Please go to Data Inisight > Policy Compliance to unlock the product
...
C035: No brand cannot be selected

 | 

Can't set Brand name as No brand, Mandatory select Brand due to LazMall identity
...
BIZ\_CHECK\_SPECIAL\_PRICE\_GREATER\_THAN\_PRICE

 | 

The discount price must be cheaper than the regular price
...
THD\_IC\_ERR\_F\_DOMAIN\_IMAGE\_00\_01\_003

 | 

Main image has duplicate. Please remove to continue
...
THD\_IC\_ERR\_F\_PRODUCT\_00\_15\_004

 | 

The max length of Multi title for locale can not exceed 255 bytes
...
VARIATION\_CATEGORY\_ATTRIBUTE\_INVALID

 | 

This variaition attribute is not found in the category attribute library
...
THD\_IC\_ERR\_F\_IC\_ABILITY\_PG
...
BIZ\_CHECK\_RESTRICTED\_CATEGORY

 | 

You are not authorised to sell this category
...
BIZ\_CHECK\_MAX\_PACKAGE\_WEIGHT

 | 

The package weight exceeds 40kg, please make change or contact your category manager to apply DBS permission.
...
BIZ\_CHECK\_MAX\_PACKAGE\_DIMENISIONS

 | 

The Package dimension(Length+Width+Height) exceeds 300cm, please make change or contact your category manager to apply DBS permission.
...
THD\_IC\_ERR\_F\_IC\_INFRA\_SPU\_036

 | 

eancode already exists
...
THD\_IC\_ERR\_F\_IC\_DOMAIN\_PROPERTY\_002

 | 

The value of category property is required,property
...
BIZ\_CHECK\_BRAND\_PERMISSION\_TIER\_TWO

 | 

Tienes que solicitar dicha marca para poder utilizarla.
...
CHK\_IMAGE\_MAX\_ITEMS

 | 

Upload a maximum of 8 pictures
...
BIZ\_CHECK\_MTEE\_RISK\_RULE\_TRIGGER\_MTEE\_RISK\_RULE\_TRIGGER\_ERROR

 | 

The price or content of this product violates the current national policy, please modify it according to the relevant policy mentioned in the details of the message field.
...
BIZ\_CHECK\_MAIN\_IMAGE\_REQUIRE:Main image is require

 | 

Please add at least one main or SKU image of the item. lazada does not allow uploading items without images.
...
The store you requested is a Lazmall store, there are strict restrictions on the use of brands for this type of store, please apply for permission to use the relevant brand on the Seller Center -> setting -> brand page.
...
The values of variant attributes are duplicated between SKUs. Multi-SKU products need to set variant attributes and make sure that the values of variant attributes owned by each SKU cannot be duplicated.
