...
> Source: https://partner.tiktokshop.com/docv2/page/widget-sdk-user-guide
> Section: Developer Guide
> Scraped: 2026-05-21T00:26:38.291Z
...
Added init method to support user integration and initial widgets.
...
Added loadWidget method to support users loading our widget page.
...
Added update method to support users to modify widget config.
...
Added preloadWidget method to support users to preload widget resources.
...
Added WidgetConfigProvider component, the component integrates the original init, update, preload methods, WidgetConfigProvider should be used with RemoteWidget, it is recommended to use WidgetConfigProvider in the react integration scenario instead of directly calling init, update, preload.
...
seller id, used for event tracking
...
seller region, fill in the standard code of the country where the merchant is located, such as 'US', 'GB', and so on.
...
authorized by the service market, used for authentication
...
'@tiktokshop-widget/product:optimizer'

 | 

Integrated widget name, provided by the open platform
...
`JSON <br> const Component = loadWidget(name); <br> return ( <br> <div> <br> <Component /> <br> </div> <br> ); <br>`
...
`JSON <br> loadWidget(name).then((component: WidgetComponent) => { <br> view = component.create( <br> document.getElementById('widget-preview-container'), <br> ); <br> view?.render({}); <br> }); <br> <br> return ( <br> <div <br> className="widget-preview-container" <br> id="widget-preview-container"></div> <br> ); <br>`
...
(props: Record<string, any>) => Promise
...
seller region, used for traffic scheduling
...
seller region, fill in the standard code of the country where the merchant is located, such as 'US', 'GB', 'ID', and so on.
...
Configuration of incoming widget components
...
You need to initialize [init](https://bytedance.larkoffice.com/docx/VmDKdqmlCokkwcx5srecegJongc#doxcnZJv54tgHadJdfTbSydf4nc) first before using it

 |
| 

Current name configuration not found

 | 

Incorrect resource loading, please check if the resource is abnormal
...
![](https://p16-arcosite-sg.ibyteimg.com/tos-alisg-i-k9wyc2ijk0-sg/48565dbe760d4a4783c023756b8bc146~tplv-k9wyc2ijk0-image.image)
...
![](https://p16-arcosite-sg.ibyteimg.com/tos-alisg-i-k9wyc2ijk0-sg/e51e5883f9ab45eabb29dc106e834b50~tplv-k9wyc2ijk0-image.image)
...
1.  Interface/api/v1/seller/widget/get error list
...
Widget domain not match

 | 

The domain name configured in the whitelist does not match the actual one.
...
Widget Token Is Expired

 | 

The widget token has expired
...
Widget token is invalid

 | 

The widget token was sent incorrectly
...
Widget Token Shop Not Match

 | 

Your input shopid is not consistent with the shop info in widget.
...
Appkey whitelist is not configured. You need to provide the domain name and appKey to the open platform and configure the whitelist.
...
The shop region is not matched with server room. In most cases, it was caused by unmatched shop\_id and region.

 |

1.  The error is reported as follows

![](https://p16-arcosite-sg.ibyteimg.com/tos-alisg-i-k9wyc2ijk0-sg/fcd2300daa394c109f177c97f0d291b2~tplv-k9wyc2ijk0-image.image)
