...
> Section: Changelog
> Scraped: 2026-05-21T00:37:14.253Z
...
An advance order for items that are not yet available or released. **Fulfillment starts on a specific date in the future.**
...
An order for items that are produced only after the order is received. **Fulfillment starts after the product is produced.**
...
An order for items that are out of stock but expected to be restocked. **Fulfillment starts after the product is restocked.**
...
The date an order should arrive by to be considered on-time and to avoid late delivery penalties.
...
The date on which order handling starts and the status of the order changes to [AWAITING\_SHIPMENT](order-api-overview). Applicable only if the `order_type` is `PRE_ORDER`. **Note**: When a pre-order status is `ON_HOLD`, the product is still awaiting release, so payment will only be authorized 1 day before the release.
...
The duration for the seller to process the order and hand it over to a shipping carrier after the order is placed. Applicable only if the `order_type` is `MADE_TO_ORDER` or `BACK_ORDER`.
