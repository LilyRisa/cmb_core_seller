...
class AdminInvoiceRefundTest extends TestCase
{
    ...
    public function test_admin_creates_manual_invoice(): void
    {
        ...
        $this->actingAs($admin, 'admin_web')->postJson("/api/v1/admin/tenants/{$tenant->id}/invoices", [
    ...
    public function test_mark_paid_creates_payment_and_fires_invoice_paid(): void
    {
        ...
        $this->actingAs($admin, 'admin_web')->postJson("/api/v1/admin/invoices/{$invoice->id}/mark-paid", [
    ...
    public function test_mark_paid_idempotent_on_already_paid_invoice(): void
    {
        ...
        $this->actingAs($admin, 'admin_web')->postJson("/api/v1/admin/invoices/{$invoice->id}/mark-paid", [])->assertOk();
        // 2nd call — idempotent
        $this->actingAs($admin, 'admin_web')->postJson("/api/v1/admin/invoices/{$invoice->id}/mark-paid", [])->assertOk();
    ...
    public function test_refund_payment_marks_refunded_and_optionally_rolls_back_subscription(): void
    {
        ...
        $this->actingAs($admin, 'admin_web')->postJson("/api/v1/admin/payments/{$payment->id}/refund", [
