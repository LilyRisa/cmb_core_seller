...
import { CarOutlined, ReloadOutlined, SearchOutlined, SyncOutlined, WarningOutlined } from '@ant-design/icons';
...
export function OrdersPage() {
    ...
    }), [effectiveStatus, q, skuQ, productQ, channelAccountId, carrier, placedFrom, placedTo, tabKey, params, sort, page, perPage]);

    // stats reflect everything except the status tab and the carrier card selection
    ...
    }), [q, skuQ, productQ, channelAccountId, placedFrom, placedTo]);
    ...
    const shopName = (id: number | null) => accounts.find((a) => a.id === id)?.name;
            ...
            render: (_, o) => (
                <Space direction="vertical" size={2}>
                    ...
                    <Space size={4} wrap>
                        ...
                        {(o.channel_account?.name ?? shopName(o.channel_account_id)) && <Tag>{o.channel_account?.name ?? shopName(o.channel_account_id)}</Tag>}
    ...
    const carrierCards = stats?.by_carrier ?? [];
        ...
        <div>
            ...
            {carrierCards.length > 0 && (
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>
                    <CarrierCard label="Tất cả ĐVVC" icon count={(stats?.total ?? 0)} active={!carrier} onClick={() => set({ carrier: undefined })} />
                    {carrierCards.map((c) => (
                        <CarrierCard key={c.carrier} label={c.carrier} count={c.count} active={carrier === c.carrier} onClick={() => set({ carrier: c.carrier })} />
                    ))}
                </div>
            )}
            ...
            <Card style={{ marginTop: 12 }} styles={{ body: { padding: 16 } }}>
                <Space wrap style={{ marginBottom: 12 }}>
                    <Input.Search allowClear placeholder="Mã đơn / tên người mua" prefix={<SearchOutlined />} style={{ width: 230 }} defaultValue={q} onSearch={(v) => set({ q: v || undefined })} />
                    <Input.Search allowClear placeholder="Mã SKU" style={{ width: 150 }} defaultValue={skuQ} onSearch={(v) => set({ sku: v || undefined })} />
                    <Input.Search allowClear placeholder="Tên sản phẩm" style={{ width: 180 }} defaultValue={productQ} onSearch={(v) => set({ product: v || undefined })} />
                    <Select allowClear placeholder="Tất cả gian hàng" style={{ width: 220 }} value={channelAccountId || undefined} onChange={(v) => set({ channel_account_id: v })}
                        options={accounts.map((a) => ({ value: a.id, label: `${a.name} (${a.provider})` }))} />
                    {carrierCards.length > 0 && (
                        <Select allowClear placeholder="Đơn vị vận chuyển" style={{ width: 200 }} value={carrier || undefined} onChange={(v) => set({ carrier: v })}
                            options={carrierCards.map((c) => ({ value: c.carrier, label: `${c.carrier} (${c.count})` }))} />
                    )}
                    <RangePicker
                    ...
                    />
                    <Select value={sort} style={{ width: 170 }} onChange={(v) => set({ sort: v })} options={[
...
function CarrierCard({ label, count, active, onClick, icon }: { label: string; count: number; active: boolean; onClick: () => void; icon?: boolean }) {
    return (
        <Card
            size="small" hoverable onClick={onClick}
            styles={{ body: { padding: '10px 14px' } }}
            style={{ minWidth: 140, cursor: 'pointer', borderColor: active ? '#1668dc' : undefined, background: active ? '#e6f4ff' : undefined }}
        >
            <Space direction="vertical" size={0}>
                <Typography.Text type="secondary" style={{ fontSize: 12 }}>{icon && <CarOutlined style={{ marginRight: 4 }} />}{label}</Typography.Text>
                <Typography.Text strong style={{ fontSize: 20 }}>{count}</Typography.Text>
            </Space>
        </Card>
    );
}
