...
- **CẦN LÀM KHI DEPLOY (chưa verify được ngoài staging):** cấu hình reverse proxy (NPM/Caddy) route **`wss://<domain>/app` → `cmb-reverb:8080`** kèm header `Upgrade`/`Connection: upgrade` (chỉ `/app`; `/apps` là API nội bộ app→reverb, không expose). Hướng dẫn chi tiết: `docs/07-infra/reverb-realtime-setup.md`. Thiếu bước này realtime không chạy nhưng FE vẫn polling (không vỡ).
