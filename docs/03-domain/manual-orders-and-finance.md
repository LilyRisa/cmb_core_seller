...
- **Thanh toán & COD (cập nhật 2026-06-03):** COD (tiền thu hộ qua ĐVVC) = **số tiền còn thiếu cuối cùng** = `grand_total − prepaid_amount` (kẹp ≥ 0), tính **tự động** trong `ManualOrderService::create/update`. **Không** còn checkbox `is_cod` riêng trên FE — checkbox cũ default off từng khiến đơn lưu `cod_amount=0` ⇒ đẩy GHN COD 0đ dù khách phải trả khi nhận. `is_cod` nay được **suy ra** (`còn thiếu > 0`); `payment_status` = `paid` (trả đủ) / `partial` (trả một phần) / `cod` (còn thu hộ, chưa trả trước) / `unpaid`. Nếu trả trước **vượt** giá trị đơn ⇒ còn thiếu âm ⇒ COD kẹp **0đ** và FE hiện **cảnh báo**. Khi "Chuẩn bị hàng", `ShipmentService::createForOrder` đẩy đúng số còn thiếu này cho ĐVVC (âm ⇒ 0đ + log `shipment.cod_overpaid_clamped_to_zero`).
