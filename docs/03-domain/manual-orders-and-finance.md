...
- **Tạo đơn:** màn "Tạo đơn" trên SPA: chọn nguồn phụ (`manual` / `website` / `facebook` / `zalo` / `hotline` — chỉ là nhãn), nhập khách hàng (tên, SĐT, địa chỉ tỉnh/huyện/xã VN), thêm dòng hàng = chọn **master SKU** (tìm theo code/tên/barcode) + số lượng + đơn giá (mặc định từ giá bán/giá listing nếu có) + chiết khấu, nhập phí ship / COD / ghi chú / tag.
- **Lưu đơn:** tạo `orders(source=manual, channel_account_id=null, external_order_id=null, order_number=tự sinh)` + `order_items(sku_id đã có)`; trạng thái mặc định `pending` (hoặc `processing` nếu chọn) ⇒ **reserve tồn ngay** (movements `order_reserve`).
