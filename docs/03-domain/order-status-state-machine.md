...
7. **"Sticky-forward" cho ĐƠN SÀN — tối ưu luồng chờ (độ trễ async của sàn).** Khi app đã "Chuẩn bị hàng" (local đã ở `processing`/`ready_to_ship`), `OrderUpsertService` **KHÔNG để đồng bộ ngược kéo trạng thái LÙI** về `pending`/`unpaid` chỉ vì sàn chưa kịp cập nhật — vd Shopee `READY_TO_SHIP→PROCESSED` là bất đồng bộ, mà `READY_TO_SHIP` map ra `pending`, nên poll kế dễ "kéo đơn nhảy về Chờ xử lý". Đồng thời **KHÔNG auto-nhảy `processing → ready_to_ship` từ sync** (chỉ `ShipmentService::markPacked` nội bộ mới đẩy — giữ đúng mô hình 2 bước). Chỉ GIỮ trạng thái — KHÔNG bao giờ chặn tiến thật (`shipped`/`delivered`/`cancelled`/`returning` vẫn áp bình thường); đơn MỚI (created) không bị ảnh hưởng. Vẫn lưu `raw_status` thật. Tắt bằng `SYNC_HOLD_CHANNEL_READY_TO_SHIP=false`. Đây là chốt chặn chính cho hiện tượng "trạng thái nhảy lung tung" của đơn sàn.
