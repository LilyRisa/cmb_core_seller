...
7. **`ready_to_ship` là mốc NỘI BỘ (markPacked), KHÔNG auto từ sync.** Một số sàn tự đẩy đơn lên `ready_to_ship` ngay sau bước "Chuẩn bị hàng" (vd Lazada `/order/pack` với vài delivery_type). Để giữ đúng mô hình 2 bước ("Chuẩn bị hàng"→`processing`; "Đã gói & sẵn sàng bàn giao" (`ShipmentService::markPacked`)→`ready_to_ship`), `OrderUpsertService` **KHÔNG auto-nhảy `processing → ready_to_ship` khi đồng bộ ngược từ sàn** (chỉ giữ `processing`, vẫn lưu `raw_status` thật). Đơn đã ở `ready_to_ship` (do markPacked) sẽ KHÔNG bị sync kéo lùi (config map `ready_to_ship→ready_to_ship` nên cùng trạng thái = no-op). Tắt bằng `SYNC_HOLD_CHANNEL_READY_TO_SHIP=false`. Đơn MỚI đồng bộ về mà sàn đã ở `ready_to_ship` (chưa từng "Chuẩn bị hàng" trong app) KHÔNG bị giữ.
