...
2. **Không mask SĐT ở màn nội bộ** (SPEC 2026-07-15) — chủ shop phải làm chủ dữ liệu khách hàng của chính họ; mọi role có quyền `customers.view`/`orders.view`/`accounting.view` tương ứng đều thấy `phone`/`buyer_phone` đầy đủ ở `CustomerResource`/`OrderResource`/`PartyController` (không còn permission `customers.view_phone`, đã xoá khỏi catalog). Log không chứa SĐT/địa chỉ đầy đủ. **Ngoại lệ:** trang tra cứu vận đơn **công khai** (`PublicTrackingService`, không đăng nhập, xem bởi bên thứ 3/khách hàng) vẫn mask — đây là bảo vệ người mua trước bên ngoài, không phải dữ liệu nội bộ của chủ shop.
...
5. **Quyền truy cập PII** chỉ cấp cho role có quyền view module tương ứng (không còn phân theo "xem SĐT che/đầy đủ" — xem điểm 2). `customer_notes` mà NV gõ vào có thể vô tình chứa SĐT/email — khi anonymize, regex redact các pattern PII trong `note` text.
6. **Không chia sẻ PII** ra ngoài hệ thống (không gửi sang dịch vụ thứ ba không cần thiết). Khi tải label PDF từ ĐVVC/sàn (chứa PII) ⇒ lưu MinIO theo tenant, chỉ tải qua signed URL hết hạn ngắn.
...
6. **Không mask SĐT khi hiển thị nội bộ** (SPEC 2026-07-15) — SĐT trong `conversations.buyer_phone`/inline trong `messages.body` hiển thị đầy đủ cho user có quyền `messaging.view`, cùng chính sách với §6 điểm 2.
