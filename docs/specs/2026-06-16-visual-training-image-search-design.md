...
- `id`, `tenant_id` (BelongsToTenant), `name`, `description` (nullable), `attributes` (json — đặc điểm/ghi chú), `ref_code` (nullable, mã tham chiếu tùy chọn), `status` ('active'|'indexing'|'failed'), `applies_all_pages` (bool, default true), `primary_image_id` (nullable FK → `visual_training_images`, **ảnh đại diện/cover**), `created_by`, timestamps, softDeletes.
- Pivot `visual_training_item_page` (item_id, channel_account_id, tenant_id) — scoping per-page như SPEC-0035.

**Ảnh đại diện (`primary_image_id`):** mỗi item có 1 ảnh đại diện để (a) hiển thị nhanh trên UI, (b) làm **ảnh ứng viên duy nhất gửi cho vision re-rank** (nhanh & rẻ hơn gửi nhiều ảnh). Mặc định = ảnh đầu tiên upload; seller đổi được. Xóa ảnh đại diện ⇒ tự trỏ về ảnh còn lại đầu tiên (hoặc null nếu item trống).
...
7. Quyết định trạng thái:
   ...
   - **Có re-rank** (`opts.rerank` && đủ credit && có candidate): `VisionReRanker.analyzeImages(ảnh khách + **ảnh đại diện** (`primary_image_id`, fallback ảnh đầu) của top-N item + name/desc)` → chọn `item_id` hoặc `"none"` + confidence:
...
- Vision re-rank gửi **ảnh đại diện** (`primary_image_id`) mỗi item (1 ảnh/item) để nhanh & rẻ; có thể nới thêm ảnh phụ qua config nếu cần độ chính xác cao hơn.
