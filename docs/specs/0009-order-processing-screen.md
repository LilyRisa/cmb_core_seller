...
- **Chọn xuyên trang:** server phân trang ⇒ checkbox header chỉ chọn trang hiện tại. Thêm dropdown chọn ở cột checkbox: **"Chọn trang hiện tại"**, **"Chọn tất cả N đơn (mọi trang)"** (chỉ hiện khi >1 trang) và "Bỏ chọn". "Mọi trang" gọi `useFetchAllOrders` (lặp `/orders` per_page=100) tải tối đa **500 đơn/lượt** (báo nếu lô lớn hơn). Bulk action lấy object đơn từ **`orderCache`** (gộp các trang đã xem + lô fetch) nên xử lý đúng cả đơn KHÔNG ở trang hiện tại; `preserveSelectedRowKeys` giữ chọn khi lật trang. Đổi bộ lọc/tab ⇒ tự bỏ chọn.
