...
- FE: trang `/fulfillment` đổi thành **"Xử lý đơn hàng"** — chips nền tảng (TikTok/Shopee/Lazada/Đơn tay, có màu/icon) ở trên cùng + 2 ô lọc (khách hàng / sản phẩm) + 3 tab bước (`Cần xử lý (n)` / `Chờ đóng gói (n)` / `Chờ bàn giao (n)`) + tab "Vận đơn" + tab "Quét" (Segmented Đóng gói/Bàn giao). Badge "đã in N×" trên mỗi dòng; nút "In tem" **disable + tooltip** khi bộ chọn lẫn nền tảng/ĐVVC; in lại (chọn có đơn `print_count≥1`) ⇒ **popup xác nhận** trước khi in.
**Ngoài (follow-up):**
...
**UI** (`resources/js/pages/FulfillmentPage.tsx`): PageHeader "Xử lý đơn hàng" + thanh tiến trình print job; trên cùng (cho 3 tab bước): **chips nền tảng** (`Tag.CheckableTag` — `Tất cả` / TikTok / Shopee / Lazada / Đơn tay, màu theo `CHANNEL_META`) + 2 `Input.Search` ("Lọc theo khách hàng", "Lọc theo sản phẩm"); 3 tab bước (Badge số lượng), mỗi tab = bảng `Order` + `rowSelection` + hành động theo bước (Tạo vận đơn / In tem / Đóng gói / Bàn giao / Picking / Packing); cột "Vận đơn" hiện `ĐVVC + trạng thái + 🖨 N× + mã tracking`; nút "In tem" disable + tooltip khi lẫn nền tảng/ĐVVC, popup xác nhận khi in lại. Tab "Vận đơn" (danh sách shipments + track/huỷ/đóng gói/bàn giao/in tem). Tab "Quét" có `Segmented` (Đóng gói / Bàn giao ĐVVC) + ô quét auto-focus + log phiên. Sidebar mục "Giao hàng & in" → `/fulfillment`.
