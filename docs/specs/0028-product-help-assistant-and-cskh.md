...
- **Cập nhật 2026-05-31 — CSKH chuyển sang HỘI THOẠI nhiều tin + đính kèm file/ảnh/video:**
  ...
  `SupportMediaService` — **không** import Services của Messaging (luật module). Disk = `SUPPORT_MEDIA_DISK`
  (prod đặt **r2** trong `docker-compose.prod.yml`, như media tin nhắn). Upload set `ContentType` đúng + ảnh/video
  thêm `ContentDisposition: inline` ⇒ browser **xem trực tiếp** ảnh (thumbnail `<img>`) / video (`<video controls>`)
  qua signed URL, không ép tải về. Domain logic gom vào
  ...
  - **FE:** tách `CskhTab.tsx` + `SupportAttachmentList.tsx` (ảnh = Ant `Image` + `PreviewGroup` ấn phóng to/zoom,
    video `<video controls>`, file = link tải). **Phía user, hội thoại ĐÃ ĐÓNG không hiển thị lại tin cũ — chỉ
    hiện mã `#id` + thời điểm đóng** (admin vẫn xem đầy đủ lịch sử). Trang admin `support-requests` (giữ route/menu)
    đổi sang xem thread + nhắn nhiều tin + đính kèm + đóng.
