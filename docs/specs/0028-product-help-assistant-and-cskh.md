...
- **Cập nhật 2026-05-31 — trang riêng `/admin/ai-support` + provider embedding tách rời:**
  gom toàn bộ cấu hình trợ lý Hỏi AI vào trang admin riêng (`AdminAiSupportPage`, menu "AI Trợ giúp"),
  KHÔNG lẫn với cấu hình provider messaging. Thêm key `help_assistant.embedding_provider_code`
  (+ config `support.assistant.embedding_provider_code`, env `HELP_ASSISTANT_EMBEDDING_PROVIDER`):
  cho phép tách **provider chat** khỏi **provider embedding** — cần khi provider chat không có
  embeddings API (vd OpenRouter dùng cho chat, OpenAI `text-embedding-3-small` dùng cho vector).
  `HelpAssistant::embed()` + `HelpIndexer::embeddingConnector()` ưu tiên `embedding_provider_code`,
  rỗng ⇒ fallback `provider_code`. Trang cảnh báo khi provider embedding đang chọn không hỗ trợ embedding.
