...
5. **Frontend = tổng quát hoá bài bản**: trang "Kênh tin nhắn" thành view **đa provider** dựa trên capability map, bỏ giả định cứng `facebook_page`.
...
## 12. Frontend — tổng quát hoá đa provider

Hiện trang Kênh + vài chỗ inbox **filter cứng `provider='facebook_page'`** (`MessagingChannelController::index`, GET verify, FE). Tổng quát hoá:
- **API:** `GET /api/v1/messaging/channels` trả mọi messaging provider (không lọc Facebook); mỗi kênh kèm `provider`, `capabilities` (từ `GET /messaging/capabilities`).
- **FE trang Kênh:** view đa-provider, render theo capability (ví dụ chỉ Facebook hiện post picker/comment; Zalo hiện kết nối OA + flow inbox). Nút **"Kết nối Zalo OA"** (icon font, không emoji — [[ui-use-font-icons-not-emoji]]).
- **Inbox:** hiển thị hội thoại Zalo cạnh các kênh khác; badge/nhãn provider; composer tôn trọng `outboundWindow` + capability (ẩn nút interactive nếu kênh không hỗ trợ — dùng disable không ẩn theo [[ui-order-actions-toolbar]] nơi phù hợp).
- **Flow editor:** đã provider-agnostic; chỉ cần cho chọn `provider='zalo_oa'` khi tạo flow và lọc trigger phù hợp (`inbox_first_message/keyword/any`; ẩn `comment_*` cho Zalo).
- Tránh giả định Facebook trong route guard FE; dùng `useCurrentTenantId()` ([[fe-tenant-id-use-hook-with-fallback]]).
