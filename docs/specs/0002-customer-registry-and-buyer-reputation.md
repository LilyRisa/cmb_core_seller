...
- [ ] Migration `customers` + `customer_notes` + thêm cột `orders.customer_id` (reversible).
- [ ] Module `Customers` skeleton: `Customer`/`CustomerNote` model + `CustomersServiceProvider` + `Contracts/CustomerProfileContract` + bind.
- [ ] `CustomerPhoneNormalizer` + test bảng vector.
- [ ] `ReputationCalculator` + test.
- [ ] Listener `LinkOrderToCustomer` (queue `customers`, afterCommit, ShouldBeUnique per customer_id) + test idempotent.
- [ ] Listener anonymize cho `data_deletion` + disconnect shop.
- [ ] Command `php artisan customers:backfill` (one-shot — duyệt mọi `orders` không có `customer_id`, dispatch `LinkOrderToCustomer` per order; thông báo tiến độ).
- [ ] Command scheduled `customers:recompute-stale` (mỗi giờ — recompute cho khách có order updated trong giờ qua mà chưa recompute).
- [ ] API: `GET /customers`, `GET /customers/{id}`, `GET /customers/{id}/orders`, `POST /customers/{id}/notes`, `DELETE /customers/{id}/notes/{noteId}`, `POST /customers/{id}/block`, `POST /customers/{id}/unblock`, `POST /customers/{id}/tags`, `POST /customers/merge` + permission middleware.
- [ ] Mở rộng `OrderResource` với field `customer` (qua `CustomerProfileContract`); UI `OrderDetailPage` render `<CustomerSummaryCard>`.
- [ ] FE trang `/customers` + `/customers/:id` + badge ở `OrdersPage`.
- [ ] Permissions mới (`customers.view`, `customers.note`, `customers.block`, `customers.merge`, `customers.view_phone`) trong `Role` enum + permission map (tài liệu hoá ở `01-architecture/multi-tenancy-and-rbac.md`).
- [ ] Test (unit + feature + smoke FE) — cover §9.
- [ ] Tài liệu cập nhật: spec này (Implemented), `03-domain/customers-and-buyer-reputation.md`, `01-architecture/modules.md` (thêm `Customers`), `02-data-model/overview.md` (thêm section), `05-api/endpoints.md` (gỡ khỏi "Sắp có"), `07-infra/queues-and-scheduler.md` (queue `customers` + job mới), `08-security-and-privacy.md` (§6 mới về phone-hash matching), `00-overview/glossary.md` (thêm Customer / Reputation / Phone hash).
