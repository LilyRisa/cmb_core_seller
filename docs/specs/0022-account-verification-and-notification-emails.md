...
- **Trong:**
  ...
  - **FE gate (bổ sung 2026-05-16):** `RequireAuth` chặn user `email_verified_at=null` → render `VerifyEmailPage` thay AppLayout; user KHÔNG vào được tính năng nào khác. `VerifyEmailPage.tsx` hiện 4 bước (mở hộp thư → tìm email → **kiểm tra Spam/Quảng cáo (Gmail tab, Outlook Junk, Yahoo Bulk)** → click link/copy URL), nút "Gửi lại" cooldown 60s, "Tôi đã xác thực — Tải lại", logout/helpdesk. `EmailVerifiedPage.tsx` (route `/email-verified` public) render success/already/invalid theo `?status=` từ BE; auto refetch `/auth/me`. Hook `useResendVerification()` trong `lib/auth.tsx`.
  - **FE quên/đặt lại mật khẩu (bổ sung 2026-05-30):** `ForgotPasswordPage.tsx` (route `/forgot-password` public) nhập email → `useForgotPassword()` gọi `POST /auth/password/forgot`, luôn hiện thông báo generic "đã gửi nếu email tồn tại". `ResetPasswordPage.tsx` (route `/password-reset` public) đọc `token`+`email` từ query → form mật khẩu mới (validate FE khớp policy BE: ≥8 + hoa + thường + ký tự đặc biệt) → `useResetPassword()` gọi `POST /auth/password/reset`; xử lý `INVALID_RESET_TOKEN` (link hết hạn → CTA gửi lại) và thành công (CTA đăng nhập). `LoginPage` có link "Quên mật khẩu?". Hook `useForgotPassword()` / `useResetPassword()` + helper `errorCode()` trong `lib/api.ts`.
...
- **Password policy (reset):** mật khẩu mới phải đạt `Password::min(8)->mixedCase()->symbols()` — tối thiểu 8 ký tự, có ít nhất 1 chữ hoa, 1 chữ thường và 1 ký tự đặc biệt. Không đạt ⇒ `422 VALIDATION_FAILED` (message tiếng Việt phủ qua `lang/vi/validation.php` key `password.mixed` / `password.symbols`). Đồng bộ với policy của `POST /auth/register`.
- **Throttle endpoint:**
