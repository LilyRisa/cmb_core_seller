...
- **Trạng thái:** Implemented (2026-05-15 — core tồn kho + đơn thủ công + ghép SKU + auto-match + PushStockToChannel + TikTok `fetchListings`/`updateStock`; còn lại: sửa line-item đơn manual, WMS đầy đủ Phase 5, FE Vitest — xem §10/§11)
...
- [x] TikTok `updateStock` (capability `listings.updateStock=true`) + `fetchListings` (capability `listings.fetch=true`; `/product/202309/products/search` → flatten product→sku → `ChannelListingDTO[]`; endpoint config-able qua `integrations.tiktok.endpoints.*` — *shape cần xác nhận với Partner API*); `ChannelConnector` thêm `fetchListings()` + `updateStock($context)`; DTO `ChannelListingDTO` + `TikTokMappers::listings()`. Job `Channels\Jobs\FetchChannelListings` (queue `listings`, ShouldBeUnique/shop) — upsert `channel_listings` by `(channel_account_id, external_sku_id)` rồi `SkuMappingService::autoMatchUnmapped`. API `POST /channel-accounts/{id}/resync-listings` + `POST /channel-listings/sync`. Scheduler: hằng ngày cho mọi shop hỗ trợ `listings.fetch`. Command `inventory:resync-order-skus` (re-resolve `order_items.sku_id` cho đơn còn `has_issue='SKU chưa ghép'` sau khi có mapping mới).
