...
Trang sửa (`MarketplaceEditPage`): có nút **Quay lại**, bố cục Card chuẩn (Thông tin / Hình ảnh / Giá theo SKU), nút **Lưu & đẩy lên sàn** (chỉ bật khi có thay đổi). **Không chặn UI bằng lời gọi live API**: form seed ngay từ dữ liệu `ChannelListing` truyền qua navigation state; `getListingDetail` tải ở nền (`retry:false`) để bổ sung mô tả/mọi ảnh/mọi SKU — chậm/lỗi chỉ hiện cảnh báo, vẫn sửa được phần cơ bản. Phần Hình ảnh kèm **trình resize ảnh** (`ImageResizer`): chọn ảnh từ máy → resize bằng canvas (preset vuông 1:1 / dọc 3:4 / gốc-theo-chiều-rộng, vẽ "cover" giữ tâm) → upload `/media/image` → thêm URL vào danh sách ảnh. Tồn hiển thị read-only kèm ghi chú đẩy theo SKU.
