...
- FE: 3 tab "công việc" đầu (`Chờ xử lý`/`Đang xử lý`/`Chờ bàn giao`, key `prepare`/`pack`/`handover`) lọc theo `?stage=` (badge từ `stats.by_stage`); các tab còn lại theo trạng thái. Cột **"Thao tác"** trên danh sách đơn (`OrderActions`) — hiện nút theo **vận đơn** (không theo trạng thái đơn): **chưa có vận đơn** ⇒ "Chuẩn bị hàng (lấy phiếu)" (hoặc "Hết hàng" disable nếu `out_of_stock`); vận đơn `pending`/`created` ⇒ "In phiếu giao hàng" / "Đã gói & sẵn sàng bàn giao" / "In tem sàn" (nếu có tem thật của sàn); vận đơn `packed` ⇒ "Bàn giao ĐVVC" / "In lại phiếu"; luôn ⇒ "In hoá đơn". Tab "Chờ xử lý" cho **tick chọn nhiều đơn + nút "Chuẩn bị hàng (N)"** (bulk-create vận đơn). Tab **"Hết hàng"**. Nút "Quét đơn" (header) mở modal đóng gói/bàn giao. Icon dùng `@ant-design/icons` (không emoji — `docs/06-frontend/overview.md` §4).
