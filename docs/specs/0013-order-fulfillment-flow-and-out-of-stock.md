...
- FE: 3 tab "công việc" (`Chờ xử lý`/`Đang xử lý`/`Chờ bàn giao`) lọc theo **trạng thái đơn** (`pending`+`unpaid` / `processing` / `ready_to_ship`) — tab luôn nhất quán với cột "Trạng thái" (filter `?stage=` chỉ còn dùng cho app/API). Cột **"Thao tác"** (`OrderActions`) hiện nút theo **vận đơn**: **chưa có vận đơn** ⇒ "Chuẩn bị hàng (lấy phiếu)" (hoặc "Hết hàng" disable nếu `out_of_stock`); vận đơn `pending`/`created` ⇒ "In phiếu giao hàng" / "Đã gói & sẵn sàng bàn giao" / "In tem sàn" (nếu có tem thật của sàn); vận đơn `packed` ⇒ "Bàn giao ĐVVC" / "In lại phiếu"; luôn ⇒ "In hoá đơn". Tab "Chờ xử lý" cho **tick chọn nhiều đơn + nút "Chuẩn bị hàng (N)"** (bulk). Tab **"Hết hàng"**. Lợi nhuận ước tính hiển thị **ngay trong cột "Tổng tiền"** (dòng "LN: ±X ₫" + tooltip phân rã, cảnh báo ⚠ nếu thiếu giá vốn) — không còn cột riêng. Nút "Quét đơn" (header) mở modal đóng gói/bàn giao. Icon dùng `@ant-design/icons` (không emoji — `docs/06-frontend/overview.md` §4).
- **"Chuẩn bị hàng" cho ĐƠN SÀN** (`ShipmentService::prepareChannelOrder`): **không** tự sinh mã vận đơn giả `MAN-…` như đơn manual — dùng mã vận đơn / AWB **của sàn**: (a) đã đồng bộ về trong `order.packages[0].trackingNo` (NB đã "sắp xếp vận chuyển" trên app sàn) ⇒ dùng luôn; (b) chưa có ⇒ gọi `arrangeShipment` ("luồng A") để cập nhật trạng thái "đã in đơn" lên sàn + lấy AWB; (c) connector chưa hỗ trợ "luồng A" ⇒ **báo lỗi rõ** ("Đơn sàn chưa có mã vận đơn — sắp xếp vận chuyển trên app sàn rồi 'Đồng bộ đơn', hoặc bật `INTEGRATIONS_<X>_FULFILLMENT`") thay vì tạo vận đơn giả. Đơn → `processing`. (Đơn manual vẫn tạo vận đơn `manual` với mã `MAN-…` như cũ.)
