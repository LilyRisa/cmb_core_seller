...
- **"Chuẩn bị hàng" cho ĐƠN SÀN** (`ShipmentService::prepareChannelOrder`): tự **đẩy trạng thái "sắp xếp vận chuyển / đã in đơn" lên sàn** (`ChannelConnector::arrangeShipment` — "luồng A"; TikTok bật mặc định sau cờ `INTEGRATIONS_TIKTOK_FULFILLMENT`, lỗi gọi sàn được bắt & gắn cờ `has_issue`, không chặn) + lấy **AWB & tem PDF thật của sàn** (`getShippingDocument`) → lưu vào kho media & gắn `shipments.label_url/label_path`. **Không** tự sinh mã/tem giả. Connector chưa hỗ trợ "luồng A" / gọi sàn lỗi ⇒ vẫn tạo vận đơn cục bộ (mã có thể trống, gắn cờ `has_issue` nhắc nhở) — không chặn. Đơn → `processing`. (Đơn manual vẫn tạo vận đơn `manual` với mã `MAN-…` + phiếu giao hàng tự tạo theo khổ cài đặt.)
