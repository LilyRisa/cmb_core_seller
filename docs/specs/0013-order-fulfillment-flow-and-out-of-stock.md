...
- **"Chuẩn bị hàng" cho ĐƠN SÀN** (`ShipmentService::prepareChannelOrder`): tự **đẩy trạng thái "sắp xếp vận chuyển / đã in đơn" lên sàn** (`ChannelConnector::arrangeShipment` — "luồng A") + lấy **AWB & tem PDF thật của sàn** (`getShippingDocument`) → lưu vào kho media & gắn `shipments.label_url/label_path`. **Không** tự sinh mã/tem giả. ⚠️ "Luồng A" **TẮT mặc định** (`INTEGRATIONS_TIKTOK_FULFILLMENT=false`) — đường dẫn fulfillment "202309" cần đối chiếu Partner docs/sandbox thật (chỉnh `integrations.tiktok.endpoints.ship_package` / `TIKTOK_SHIP_PACKAGE_PATH`); khi connector chưa hỗ trợ "luồng A" hoặc gọi sàn lỗi ⇒ **vẫn tạo vận đơn cục bộ** (mã có thể trống, gắn cờ `has_issue` ngắn gọn nhắc "bật luồng A / sắp xếp trên app sàn rồi Đồng bộ đơn"), **không chặn**. `issue_reason` đã đổi thành cột `text` + cắt ≤240 ký tự (tránh tràn `varchar(255)` khi lỗi API dài). Đơn → `processing`. (Đơn manual vẫn tạo vận đơn `manual` mã `MAN-…` + phiếu giao hàng tự tạo theo khổ cài đặt.)
...
## 11. Câu hỏi mở
- ~~Khi wiring "luồng A": "Chuẩn bị hàng" có nên gọi sàn arrange-ship ngay trong nút?~~ **Quyết định:** CÓ — "Chuẩn bị hàng" **bắt buộc** cập nhật trạng thái "đã in đơn" lên sàn (`arrangeShipment`), dù phiếu in lấy từ sàn hay tự tạo. Hook đã wired trong `ShipmentService::createForOrder`; impl per-connector: TikTok đã có (sau cờ `INTEGRATIONS_TIKTOK_FULFILLMENT`, cần đối chiếu sandbox `POST /fulfillment/{ver}/orders/{order_id}/packages/ship`), Shopee/Lazada = follow-up.
