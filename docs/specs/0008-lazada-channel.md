# SPEC 0008: Lazada — connector (auth + đồng bộ đơn + listings + đẩy tồn + webhook)

- **Trạng thái:** Implemented (2026-05-17 — lõi Phase 4; RTS/AWB của Lazada + đối soát = follow-up)
...
- Khả năng (capabilities): `orders.fetch`, `orders.webhook`, `listings.fetch`, `listings.updateStock` = true; `shipping.*`, `finance.*`, `returns.*`, `listings.publish/updatePrice` = false (Phase sau — core ẩn các tính năng đó, không branch theo tên).
- Config: block `config/integrations.lazada` (hosts, version-agnostic endpoint paths cho phần khác nhau giữa sandbox/version, `status_map`, `webhook_message_types`); `INTEGRATIONS_CHANNELS` thêm `lazada`; `.env.example` thêm `LAZADA_*`; `throttle.lazada` (đã có).
- Đăng ký `LazadaConnector` trong `IntegrationsServiceProvider::$channelConnectors`.
- Doc `docs/04-channels/lazada.md`. Test contract `LazadaConnectorContractTest`.
**Ngoài (follow-up Phase 4 / Phase sau):**
- **RTS / đóng gói / AWB của Lazada** (`/logistic/...`, `/order/document/get` → base64 PDF) — `arrangeShipment`/`getShippingDocument` hiện ném `UnsupportedOperation`. Khi làm: nối vào module Fulfillment (SPEC-0006) như "luồng A — logistics của sàn".
- **Shopee connector** — spec riêng khi bắt đầu (cấu trúc tương tự).
- **Đăng/sửa listing đa sàn** (Phase 5), **đối soát settlement** (Phase 6 — `/finance/...`), **trả hàng** (Phase 7 — `/reverse/...`).
