...
> **Cập nhật 2026-05-31 — Prompt reply ưu tiên hội thoại + so khớp sản phẩm + chốt đơn:** persona + chỉ dẫn khi AI trả lời tin nhắn khách gom về `Integrations/Ai/Concerns/ReplyPersona` (dùng chung cho Claude/OpenAI-compatible/CustomHttp — trước đây lặp 3 nơi). Quy tắc tư vấn: (1) ƯU TIÊN đọc đoạn hội thoại để xác định khách hỏi SẢN PHẨM nào + cần tư vấn gì; (2) CHỈ tra `Tài liệu tham khảo` (RAG/KB) khi hội thoại thiếu thông tin; (3) phải SO KHỚP ĐÚNG sản phẩm — không lấy thông tin sản phẩm khác; (4) không chắc thì HỎI LẠI. Quy tắc CHỐT ĐƠN: (A) tư vấn xong → mời khách để lại địa chỉ + SĐT để lên đơn; (B) khách muốn tư vấn thêm → xin SĐT để nhân viên gọi trực tiếp; (C) khách đã gửi địa chỉ+SĐT và xác nhận đặt → cảm ơn + báo chú ý điện thoại nhận hàng trong 2-4 ngày; (D) không xin lại thông tin khách đã cung cấp. Áp cho cả suggest lẫn auto-mode (không áp cho intent-classify — giữ guardrail).
