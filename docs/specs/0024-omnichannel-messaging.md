...
> **Cập nhật 2026-05-31 — Prompt reply ưu tiên hội thoại + so khớp sản phẩm:** persona + chỉ dẫn khi AI trả lời tin nhắn khách gom về `Integrations/Ai/Concerns/ReplyPersona` (dùng chung cho Claude/OpenAI-compatible/CustomHttp — trước đây lặp 3 nơi). Quy tắc mới: (1) ƯU TIÊN đọc đoạn hội thoại để xác định khách hỏi SẢN PHẨM nào + cần tư vấn gì; (2) CHỈ tra `Tài liệu tham khảo` (RAG/KB) khi hội thoại thiếu thông tin; (3) phải SO KHỚP ĐÚNG sản phẩm — không lấy thông tin sản phẩm khác để trả lời; (4) không chắc sản phẩm thì HỎI LẠI thay vì đoán. Áp cho cả suggest lẫn auto-mode (không áp cho bước intent-classify — giữ guardrail).
