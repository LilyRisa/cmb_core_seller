...
| GET | `/api/v1/billing/pro-trial/eligibility` | sanctum + tenant (`billing.manage`) | — | `{ data:{ eligible:bool, reason:string\|null (mode_off\|window_closed\|already_used\|plan_too_high), duration_days:int, ends_preview:string\|null, show_popup:bool } }` — kiểm tra tenant có đủ điều kiện đăng ký trải nghiệm gói Pro. `show_popup` = có nên tự hiện popup mời trải nghiệm Pro ở FE hay không (đã tắt vĩnh viễn qua `decline` ⇒ `false`). |
...
| POST | `/api/v1/billing/pro-trial/decline` | sanctum + tenant (`billing.manage`) | — | `{ data:{ declined:true } }` — tắt vĩnh viễn popup mời trải nghiệm Pro cho tenant này (nút "Không, cảm ơn"); sau đó `eligibility.show_popup` luôn `false`. Throttle 10/phút. |
...
| Method | Path | Auth | Request | Response |
|---|---|---|---|---|
| GET | `/api/v1/admin/tenants` | web + `auth:admin_web` | query: `q`, `over_quota` (1), `suspended` (1), `page`, `per_page≤100`, `utm_source` (lọc theo acquisition->utm_source) | `{ data:[TenantSummary{id,name,slug,status,created_at,owner:{id,name,email},subscription:{plan_code,status,billing_cycle,current_period_start/end,over_quota_warned_at,over_quota_locked},usage:{channel_accounts:{used,limit,over}},acquisition:{utm_source,...}|null}], meta:{ pagination } }`. |
...
| GET | `/api/v1/admin/tenants/{tid}/product-order-stats` | web + `auth:admin_web` | query: `days` (1..365, mặc định 30), `page`, `per_page≤100` (mặc định 20), `search` | `{ data:[{sku_id,sku_code,name,mapped,order_count,qty}], meta:{ pagination } }` — số đơn (`COUNT(DISTINCT order_id)`)/số lượng bán theo sản phẩm trong N ngày (`placed_at`, fallback `created_at`), loại đơn `cancelled`/`returned_refunded`. Gộp theo `sku_id` khi đã map SKU nội bộ (`mapped=true`); chưa map gộp theo `external_product_id`+tên snapshot (`mapped=false`, `sku_id=null`). Sắp theo `order_count` giảm dần. 2026-07-24. |
