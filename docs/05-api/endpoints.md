...
| GET | `/api/v1/orders/stats` | sanctum + tenant (`orders.view`) | cùng filter như `/orders` (trừ `status`/`carrier`/`page`) | `{ data:{ total, has_issue, by_status:{ <mã>: N }, by_carrier:[{carrier,count}] } }` — dùng cho status tabs + thẻ ĐVVC. |
