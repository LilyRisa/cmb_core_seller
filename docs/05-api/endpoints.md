...
| POST | `/api/v1/orders/{id}/ship` | `fulfillment.ship` | `{ carrier_account_id?, service?, tracking_no?, cod_amount?, weight_grams?, note? }` | `201 { data: ShipmentResource }` — **"Chuẩn bị hàng / lấy phiếu giao hàng"**: tạo vận đơn (đã có shipment chưa huỷ ⇒ trả lại nó), đơn `pending → processing` (SPEC 0013 — trước đây `→ ready_to_ship`). `carrier_account_id` để trống = tài khoản mặc định, hoặc `manual`. **Đơn có SKU âm tồn (`∑on_hand−∑reserved<0`) ⇒ `422` (key `order`)** — không in phiếu khi không có hàng. Trạng thái đơn không hợp lệ (terminal/returning) ⇒ `422`. |
