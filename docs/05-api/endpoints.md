...
| POST | `/api/v1/orders/{id}/ship` | `fulfillment.ship` | `{ carrier_account_id?, service?, tracking_no?, cod_amount?, weight_grams?, note? }` | `201 { data: ShipmentResource }` — **"Chuẩn bị hàng"**: tạo vận đơn, đơn `pending → processing` (đã có shipment chưa huỷ ⇒ trả lại nó). **Đơn sàn**: tự **đẩy trạng thái "sắp xếp vận chuyển / đã in đơn" lên sàn** (`arrangeShipment` — "luồng A"; TikTok **tắt mặc định** — bật `INTEGRATIONS_TIKTOK_FULFILLMENT=true` sau khi đối chiếu đường dẫn fulfillment Partner docs) + lấy **AWB & tem PDF thật của sàn** (`getShippingDocument`) lưu vào `shipments.label_url/label_path`; **không** tự sinh mã/tem giả. "Luồng A" tắt hoặc gọi sàn lỗi ⇒ **vẫn tạo vận đơn cục bộ** (mã có thể trống) + gắn cờ `has_issue` nhắc nhở (`issue_reason` là cột `text`, cắt ≤240 ký tự) — không chặn. **Đơn manual**: `carrier_account_id` để trống = mặc định / `manual` (mã `MAN-…`). **Đơn có SKU âm tồn (`∑on_hand−∑reserved<0`) ⇒ `422` (key `order`)** — không in phiếu khi không có hàng. Trạng thái đơn không hợp lệ (terminal/returning) ⇒ `422`. (FE: đơn lợi nhuận ước tính âm ⇒ popup xác nhận trước khi gọi.) (SPEC 0013/0014) |
