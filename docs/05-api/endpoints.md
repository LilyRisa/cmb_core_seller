...
| POST | `/api/v1/orders/{id}/ship` | `fulfillment.ship` | `{ carrier_account_id?, service?, tracking_no?, cod_amount?, weight_grams?, note? }` | `201 { data: ShipmentResource }` — **"Chuẩn bị hàng"**: tạo vận đơn, đơn `pending → processing` (đã có shipment chưa huỷ ⇒ trả lại nó). **Đơn sàn**: tự **đẩy trạng thái "sắp xếp vận chuyển / đã in đơn" lên sàn** (`arrangeShipment` — "luồng A"; TikTok bật mặc định, lỗi gọi sàn được bắt & gắn cờ `has_issue` trên đơn, **không chặn** — vẫn tạo vận đơn cục bộ với mã có thể trống) + lấy **AWB & tem PDF thật của sàn** (`getShippingDocument`) lưu vào `shipments.label_url/label_path`; **không** tự sinh mã/tem giả. **Đơn manual**: `carrier_account_id` để trống = mặc định / `manual` (mã `MAN-…`). **Đơn có SKU âm tồn (`∑on_hand−∑reserved<0`) ⇒ `422` (key `order`)** — không in phiếu khi không có hàng. Trạng thái đơn không hợp lệ (terminal/returning) ⇒ `422`. (FE: đơn lợi nhuận ước tính âm ⇒ popup xác nhận trước khi gọi.) (SPEC 0013/0014) |
