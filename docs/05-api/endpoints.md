...
| GET/POST | `/api/v1/print-jobs` | `fulfillment.print` | `type?` / `{ type:'label'\|'picking'\|'packing'\|'invoice', order_ids?:[≤500], shipment_ids?:[≤500] }` | `{ data:[PrintJobResource], meta:{pagination} }` / `201 { data: PrintJobResource }` (`status=pending` → job ở queue `labels` render PDF; FE poll). `invoice` = hoá đơn / phiếu bán hàng, một trang/đơn (tên cửa hàng, mã đơn/ngày, khách & địa chỉ giao, bảng hàng + tiền, COD, ghi chú). Thiếu cả order_ids & shipment_ids ⇒ `422`. `type=label` mà bộ chọn **lẫn nhiều nền tảng / nhiều ĐVVC** ⇒ `422`. Mỗi lần in `type=label` xong ⇒ `print_count++` trên các shipment được ghép. (SPEC 0009 / 0012) |
