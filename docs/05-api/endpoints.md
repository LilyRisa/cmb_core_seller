...
| POST | `/api/v1/fulfillment/quote` | `orders.create` / `fulfillment.ship` | `{ carrier_account_id?, weight_grams (≥1), value?, recipient:{ province, district, ward?, address? } }` | `{ data:{ carrier, carrier_name, fee, insurance_fee } }` **hoặc** `{ data:null }` nếu ĐVVC không hỗ trợ tính phí (vd GHN), lỗi, hoặc chưa cấu hình tài khoản. Carrier-agnostic (`connector.quote`); hiện chỉ **GHTK** trả phí. Dùng cho gợi ý phí ở màn tạo đơn — không chặn tạo đơn. |
