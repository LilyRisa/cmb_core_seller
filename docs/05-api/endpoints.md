...
| PATCH/DELETE | `/api/v1/skus/{id}` | `products.manage` | partial (mọi trường cơ bản kể cả PIM mới; **không** sửa `mappings`/`levels` qua đây) / — | `SkuResource` / soft delete (`409` nếu còn `on_hand`/`reserved`). |
