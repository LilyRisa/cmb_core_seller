...
| GET | `/api/v1/customers/lookup` | sanctum + tenant (`customers.view`) | query: `phone` | Tra cứu nhanh khi tạo đơn thủ công. `{ data: { customer: CustomerResource\|null, addresses[], open_orders[], returning_orders[], bad_report } }`. `customer=null` nếu chưa có hồ sơ (không 404). `bad_report` (SPEC 0038, Pancake "bom hàng" — bù đắp khi nội bộ thiếu dữ liệu, trả CẢ khi `customer=null`): `null` khi tắt cấu hình/lỗi/sạch, hoặc `{ order_fail, order_success, warning_count, warnings:[{reason, reported_at}], has_data }`. Cache 24h ở `customer_bad_reports`. |
