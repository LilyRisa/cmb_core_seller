...
- Middleware: `auth:admin_web` (không có permission riêng — giống các endpoint billing admin khác).
...
`GET /api/v1/admin/invoices` — middleware `auth:admin_web` (không có permission riêng).
