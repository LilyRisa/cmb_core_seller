...
**Sửa / xoá SKU (`products.manage`):** mỗi dòng có nút ✎ (mở `Modal` "Sửa SKU" — sửa các trường cơ bản: mã/tên/SPU/danh mục/barcode/GTIN/đơn vị/giá vốn TK/giá bán TK/ngày bán/cân nặng & kích thước/ghi chú/đang hoạt động ⇒ `PATCH /skus/{id}` qua `useUpdateSku`; ghép nối SKU sàn & tồn đầu kỳ vẫn chỉnh ở tab "Liên kết SKU (sàn)" / "Tồn theo SKU") và nút 🗑 (`Popconfirm` ⇒ `DELETE /skus/{id}` qua `useDeleteSku`; chặn ở client nếu `on_hand_total`/`reserved_total` ≠ 0, server cũng trả `409`). Inputs tên/mã có `maxLength` (255 / 100) khớp validate backend.

## 6. Việc còn lại / mở rộng
- **Sửa SKU trên trang riêng:** sửa các trường cơ bản đã có (modal "Sửa SKU" ở §5); nếu muốn sửa cả ghép nối/tồn trong một trang thì tái dùng `CreateSkuPage` cho `/inventory/skus/:id/edit` (nạp `useSku(id)`, prefill — mappings/levels cần endpoint sửa riêng, chưa có).
