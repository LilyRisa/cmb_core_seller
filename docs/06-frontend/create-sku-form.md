...
## 4. Ô ảnh — TODO (chưa làm)
Mục "Thông tin cơ bản" có **`<div>` placeholder vô hiệu hoá** (icon `PictureOutlined`, tooltip "Tải ảnh sản phẩm — tính năng sắp có"), `cursor:not-allowed`. Cột `skus.image_url` luôn để trống. Khi triển khai: thay bằng AntD `<Upload listType="picture-card">` gọi endpoint upload (xem SPEC 0005 §7), hiển thị `image_url` ở bảng SKU và các nơi liệt kê hàng.
