...
## Phase 2 — Đơn thủ công + SKU + Tồn kho lõi + Sổ khách hàng  ◑ (backend xong: sổ khách + SKU/tồn/đơn tay/ghép SKU/đẩy tồn; còn FE đầy đủ + TikTok fetchListings)
...
- ☐ **Chưa làm (phần còn lại Phase 2):** TikTok `fetchListings` (lấy product/SKU của shop về `channel_listings`) + `FetchChannelListings` job + `ChannelListingDTO` · sửa line-item của đơn manual sau khi tạo · backfill SKU cho `order_items` đã có (đơn Phase 1) · cảnh báo realtime hết hàng/âm kho (Phase 6 — qua Notifications) · FE đầy đủ cho Tồn kho/Sản phẩm/Liên kết SKU/Tạo đơn tay + Vitest. (WMS đầy đủ — nhập/xuất/điều chuyển/kiểm kê/FIFO — Phase 5.)
