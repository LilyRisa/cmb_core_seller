...
- **Fulfillment "luồng A"** (Phase 4 — Implemented 2026-05-13, gated `INTEGRATIONS_LAZADA_FULFILLMENT=true` mặc định; `LAZADA_FULFILLMENT_MODE=auto|refetch_only`, mặc định `auto`):
  ...
  - `GET /order/document/get?doc_type=shippingLabel&order_item_ids=[...]` — trả `data.document.file` (base64 PDF). Chỉ khả dụng sau pack.
    - **Render khổ tem (fix 2026-06-03):** Lazada trả tem dạng **HTML** (`mime_type=text/html`). PHẢI render qua `GotenbergClient::htmlToLabelPdf` (`preferCssPageSize=true` + margin 0) — nếu dùng `htmlToPdf` thường, Gotenberg bỏ qua `@page { size }` của Lazada và ép A4 ⇒ tem nhỏ giữa giấy, thừa vùng trắng. HTML thiếu `@page` ⇒ connector bơm khổ tem `config('integrations.lazada.label_page_size')` (mặc định `100mm 150mm`). Có `@page` sẵn ⇒ giữ khổ gốc.
    - ⚠️ **Đơn DBS/SOF (Delivery By Seller / Seller Own Fleet)** — seller giao bằng ĐVVC ngoài Lazada ⇒ sàn **không cấp AWB**; `/order/document/get` trả `50008 not support operation for sof order`. Connector nhận diện (`isUnsupportedOrderTypeError`) → ném `ShippingDocumentUnavailable(terminal)` (không thử print_awb, không retry). `ShipmentService` đánh dấu `shipment.raw.label_unavailable` + `has_issue`, dừng `FetchChannelLabel`, và gate bàn giao báo đúng lý do (KHÔNG bảo "Nhận phiếu giao hàng"). VN gần như không có loại đơn này (đơn cá biệt). Phát hiện chủ động: `GetOrderItems.delivery_option_sof == 1` (fix 2026-06-03).
...
- URL `/webhook/lazada` (route đã có). `LazadaWebhookVerifier.verify()` — **theo Lazada Open Platform "Push Mechanism" chính thức** ([doc](https://open.lazada.com/apps/doc/doc?nodeId=29526&docId=120168)): chữ ký = **hex(chữ thường) HMAC-SHA256(key=`app_secret`, message = `app_key` . rawBody)**, gửi ở header **`Authorization`**. `app_key`/`app_secret` đọc qua `system_setting('marketplace.lazada.*')` (admin override) → fallback `config('integrations.lazada.*')`, **cùng nguồn với `LazadaClient`**. Fallback bảo thủ cho region/version cũ (không bắt buộc khớp): HMAC(rawBody) ở header `X-Lazop-Sign`…, hoặc body có key `sign`. Sai mọi scheme / thiếu header / chưa cấu hình secret ⇒ `verify=false` ⇒ `WebhookController` trả `401` + log `lazada.webhook.signature_invalid` (đủ chi tiết để soi scheme, KHÔNG lộ secret).
  > ⚠️ **2026-06-03 — fix `300 webhook.signature_invalid`:** scheme cũ ký `HMAC(rawBody)` (thiếu prefix `app_key`) và **loại bỏ** header `Authorization` (chỉ đọc `X-Lazop-Sign`…) — phỏng đoán SAI ⇒ 100% push fail ở `strict`. Đã chỉnh khớp đúng spec chính thức.
