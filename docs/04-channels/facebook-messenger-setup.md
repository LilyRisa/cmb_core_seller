...
2) BE trả authorize_url:
   https://www.facebook.com/v19.0/dialog/oauth?client_id=<APP_ID>
     ...
     &scope=pages_messaging,pages_manage_metadata,pages_read_engagement,pages_show_list,...,business_management
...
4) BE: verify state → đổi code lấy USER token (kèm redirect_uri GIỐNG HỆT)
   → gọi /me/accounts (page admin classic) + /me/businesses → owned_pages/client_pages
     (page thuộc Business Manager) → gộp + lấy PAGE access token từng page
...
| `business_management` | liệt kê page thuộc **Business Manager / tài khoản doanh nghiệp** (xem §5.1) |

> ### 5.1 ⭐ Page thuộc Business Manager (tài khoản doanh nghiệp) — phần hay vướng
>
> `/me/accounts` **chỉ** trả về page mà user là **admin classic** (gắn trực tiếp vào
> profile cá nhân). Page được giao qua **Business Manager / Meta Business Suite** là
> *"business asset"* nên **KHÔNG** hiện ở `/me/accounts` ⇒ "đăng nhập xong không thấy page".
>
> App xử lý bằng cách thêm quyền `business_management` rồi duyệt `/me/businesses` →
> `owned_pages` + `client_pages` từng business, gộp với `/me/accounts` (dedupe theo page id).
> Để chạy được, ngoài việc cấp `business_management` khi đăng nhập, tài khoản phải có
> **vai trò trong Business** (Business settings → People) và **page nằm trong Business** đó.
...
- **Production/Live**: cần **App Review → Advanced Access** cho `pages_messaging` +
  `business_management` (+ các quyền trên) và **Business Verification**. Chuẩn bị video
  demo luồng nhắn tin.
...
| **Đăng nhập xong không thấy page** | (a) Tài khoản không quản lý page, hoặc thiếu `pages_show_list`/chưa cấp quyền page; dev mode: phải có vai trò trong app. (b) **Page thuộc Business Manager** (tài khoản doanh nghiệp) ⇒ cấp `business_management` + đảm bảo tài khoản có vai trò trong Business và page nằm trong Business đó (xem §5.1). |
