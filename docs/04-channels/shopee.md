...
- **`create_shipping_document` → `tracking_number_invalid` / `get_shipping_parameter` → "not ready to be shipped"** (root-cause fix 2026-06-03): Shopee **PRE-ASSIGN `tracking_number` ngay ở `READY_TO_SHIP`** (trước khi seller `ship_order`) — có trong cả `get_order_detail.package_list[].tracking_number` (mapper đưa vào `packages[].trackingNo`) lẫn `get_tracking_number`. Trước đây 2 chỗ short-circuit theo tracking này ⇒ **bỏ qua `ship_order`** ⇒ đơn chưa thực sự arrange ⇒ tạo tem báo tracking invalid. Đã sửa: (a) `ShopeeConnector::arrangeShipment` quyết theo **`order_status`** (chỉ short-circuit khi PROCESSED+, ở READY_TO_SHIP LUÔN `ship_order` dù tracking đã có); (b) `ShipmentService::prepareChannelOrder` với đơn channel hỗ trợ `shipping.arrange` LUÔN gọi sàn arrange (connector idempotent) thay vì tin `packages[].trackingNo` đã sync. TikTok/Lazada không pre-assign tracking lúc pending + arrange idempotent nên KHÔNG bị ảnh hưởng. *(Nâng cấp tương lai: dùng `search_package_list` (package_status=2) để lấy package ToProcess chuẩn — xem §8.)*
