...
> **Rà soát SPEC 0013:** ánh xạ trạng thái đã đổi — "đơn sàn đã arrange/in phiếu" (TikTok `AWAITING_COLLECTION`, Shopee `PROCESSED`, Lazada `packed`/`ready_to_ship`) nay → `processing` (không phải `ready_to_ship`); `ready_to_ship` (chuẩn) **chỉ đạt được bằng thao tác nội bộ** ("đã gói & quét đơn" / `markPacked`). Bước **"Chuẩn bị hàng"** (`ShipmentService::createForOrder`): (1) **chặn nếu đơn có SKU âm tồn** (`∑on_hand−∑reserved<0` ⇒ `422`); (2) **cập nhật trạng thái "đã in đơn" lên sàn** — gọi `ChannelConnector::arrangeShipment` nếu connector hỗ trợ (`shipping.arrange`), dù phiếu in lấy từ sàn hay tự tạo; lỗi ⇒ cờ `has_issue` trên đơn, vẫn xử lý cục bộ. Hook đã wired; impl per-connector: **TikTok** đã có (`POST /fulfillment/{ver}/orders/{order_id}/packages/ship`, bật bằng `INTEGRATIONS_TIKTOK_FULFILLMENT=true` sau khi đối chiếu sandbox), **Shopee/Lazada** = follow-up; (3) tạo vận đơn (AWB/ĐVVC sàn cấp nếu có, ngược lại ĐVVC mặc định / `manual`) + sinh **phiếu giao hàng tự tạo** (`print_jobs.type=delivery` — kéo tem/AWB **thật** của sàn vẫn là "luồng A" follow-up); (4) đơn `pending → processing`. 3 "stage" `prepare/pack/handover` trong tài liệu này tương ứng 3 tab "công việc" của trang Đơn hàng (web: lọc theo **trạng thái đơn** `pending`+`unpaid` / `processing` / `ready_to_ship` — tab nhất quán với cột "Trạng thái"; filter `GET /orders?stage=` + `stats.by_stage` theo **vận đơn** giữ cho app quét đơn / API). Tab "Chờ xử lý" cho tick chọn + nút "Chuẩn bị hàng" hàng loạt. "Chuẩn bị hàng" cho **đơn sàn** dùng AWB của sàn (đồng bộ về / `arrangeShipment` "luồng A") — không tạo mã vận đơn giả; chưa lấy được AWB ⇒ báo lỗi rõ. Endpoint `/fulfillment/processing*` giữ cho app/API. Kiểm R2: `php artisan media:healthcheck`. Xem [`../specs/0013-order-fulfillment-flow-and-out-of-stock.md`](../specs/0013-order-fulfillment-flow-and-out-of-stock.md) và [`../03-domain/order-status-state-machine.md`](../03-domain/order-status-state-machine.md).
...
- **Luồng A của Lazada** (Implemented 2026-05-13 — `app/Integrations/Channels/Lazada/`, capability `shipping.arrange`/`shipping.document`=true; cờ `INTEGRATIONS_LAZADA_FULFILLMENT` + mode `LAZADA_FULFILLMENT_MODE=auto|refetch_only`):
  1. **Re-fetch order detail** (`/order/get` + `/order/items/get`, 1 lượt) — idempotent: package đã có `tracking_code` ⇒ trả tracking ngay, không pack lại.
  2. **Lọc items theo trạng thái packable** — Lazada chỉ pack được items `pending`/`topack`. Items `unpaid`/`canceled`/`packed`/`ready_to_ship`/`shipped`/`returned`/`failed`/... ⇒ skip (gọi pack sẽ bị reject `code 20 Invalid Order Item ID`). Tài liệu Lazada: *"if it is cancelled or unpaid status, then it is not allowed to be packed"*. Post-filter rỗng ⇒ ném `LazadaApiException(NoPackableItems)` với danh sách status item hiện tại để user hiểu.
  3. **Resolve `shipment_provider`** — caller truyền → config `LAZADA_DEFAULT_SHIPMENT_PROVIDER` → `/shipment/providers/get` (mặc định `dropship`, pick `is_default=true` hoặc element đầu). Cache trong-process per AuthContext.
  4. **`POST /order/pack`** — `delivery_type=dropship` (mặc định, option duy nhất cho non-FBL), `shipment_provider`, `order_items=[packable_ids]`. Response trả `tracking_number` + `package_id` + (đôi khi) `shipment_provider` cuối cùng Lazada chọn. **Recovery**: pack báo `code 20` (race condition — đơn vừa pack ở nguồn khác) ⇒ re-fetch detail; nếu có tracking thì short-circuit trả về tracking đó (không throw).
  5. **`POST /order/rts`** — `delivery_type`, `shipment_provider`, `tracking_number`, `order_item_ids=[...]`. Đẩy đơn sang RTS trên Lazada — **không bỏ qua được**, không gọi thì AWB không hợp lệ & seller bị phạt.
  6. Lấy tem (`getShippingDocument`): `/order/document/get` với `doc_type=shippingLabel` + `order_item_ids` ⇒ `data.document.file` (base64 PDF) ⇒ decode lưu kho media ⇒ `shipments.label_url`.
  7. Theo dõi: webhook push `Trade Order` (fast-path `orderRawStatus`) + polling `SyncOrdersForShop` re-fetch ⇒ `shipped → delivered`.

  Mode `refetch_only` (legacy): bỏ bước 2–5, chỉ làm bước 1+6 — cho shop không có permission "Fulfillment" trên Open Platform tự pack ngoài app.
